...
def _load():
    # Load RV features via NSQA with parquet cache for fast server startup.
    # Cache is regenerated by: python DATA/batch_update.py
    try:
        from pipeline.nsqa_data import fetch_rv_daily, compute_dte_data
        rv = fetch_rv_daily(cache_path=BASE / "features" / "rv_daily.parquet")
        rv["timestamp"] = pd.to_datetime(rv["timestamp"])
        rv["date"] = rv["timestamp"].dt.date
        dte_df = compute_dte_data(cache_path=BASE.parent / "DATA" / "NSE" / "trading_dates.csv")
    except (ImportError, ModuleNotFoundError):
        # Fallback: load directly from cached files
