...
from paths import CORE_DIR, AUTORESEARCH_DIR, RV_ROOT  # noqa: F401 — sets up sys.path
...
_META_CONFIG_FILE = AUTORESEARCH_DIR / "meta_config.json"
...
if not _META_ACTIVE:
    ...
    # ── Fixed Constants ──
    BASE_DIR = RV_ROOT
    DATA_STRATEGIES = RV_ROOT.parent / "DATA" / "Strategies"
    STRATEGY_FILES = {
    ...
    def evaluate(df: pd.DataFrame, regime_col: str = "regime_state") -> dict:
        ...
        # ── Sharpe monotonicity: within each level, Safe MUST beat Risky/Exposed ──
        # Expected ordering per level:
        #   L1: Safe > Exposed
        #   L2: Safe > Caution-A, Safe > Caution-B, Safe > Risky,
        #       Caution-A > Risky, Caution-B > Risky
        #   L3: Safe > Exposed
        EXPECTED_PAIRS = [
            ("L1 Safe", "L1 Exposed"),
        ...
        def _state_sharpe(subset, state_name):
            sub = subset[subset[regime_col] == state_name]["pnl_combined"]
            if len(sub) < 3:
                return None
            return _sharpe(sub)
        ...
        for better_state, worse_state in EXPECTED_PAIRS:
            sh_better = _state_sharpe(val, better_state)
            sh_worse = _state_sharpe(val, worse_state)
            if sh_better is not None and sh_worse is not None:
                monotonicity_checked += 1
                if sh_worse > sh_better:
                    # Violation: the "worse" state has higher Sharpe
                    monotonicity_violations += 1

        if monotonicity_checked > 0:
            monotonicity_score = 1.0 - (monotonicity_violations / monotonicity_checked)
        else:
            monotonicity_score = 0.5  # insufficient data, neutral

        composite = (
            0.30 * min(val_sharpe / 5.0, 1.5) +
            0.20 * min(safe_separation / 10.0, 1.0) +
        ...
        return {
            ...
            "monotonicity": round(monotonicity_score, 4),
