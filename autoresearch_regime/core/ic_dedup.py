...
def deduplicate_features(df: pd.DataFrame, features: list[str],
    ...
    # Sort features by predictive power (|IC| with next-day pnl) so the most
    # informative feature is always kept when a correlated pair is found.
    if "pnl_combined" in df.columns:
        ic_start = dt_date(*map(int, train_start.split("-")))
        ic_end = dt_date(*map(int, train_end.split("-")))
        train_mask = (df["date"] >= ic_start) & (df["date"] <= ic_end)
        train_df = df.loc[train_mask].dropna(subset=["pnl_combined"])
        ic_scores = {}
        for feat in available:
            feat_vals = train_df[[feat, "pnl_combined"]].dropna()
            if len(feat_vals) > 20:
                ic = abs(float(spearmanr(feat_vals[feat], feat_vals["pnl_combined"]).correlation))
                ic_scores[feat] = ic
            else:
                ic_scores[feat] = 0.0
        # Sort by |IC| descending — most predictive features survive dedup
        available = sorted(available, key=lambda f: -ic_scores.get(f, 0))
