...
class ForestManager:
    ...
    def get_summary_for_prompt(self, max_entries: int = 30, max_chars: int = 4000) -> str:
        ...
        without overwhelming context. Truncates to max_chars to prevent context explosion.
        """
        ...
        # After 50 experiments, compress failures to dimension-level summaries
        if len(self._entries) > 50:
            lines.append("**Failure patterns by dimension (compressed):**")
            for dim, stats in sorted(dim_stats.items(), key=lambda x: x[1]["total"], reverse=True):
                dim_failures = self.get_failures(dimension=dim, top_k=50)
                if dim_failures:
                    reasons = {}
                    for f in dim_failures:
                        r = f.get("reason", "unknown")[:60]
                        reasons[r] = reasons.get(r, 0) + 1
                    top_reasons = sorted(reasons.items(), key=lambda x: -x[1])[:3]
                    reasons_str = "; ".join(f"{r} ({c}x)" for r, c in top_reasons)
                    lines.append(f"  {dim}: {len(dim_failures)} failures — {reasons_str}")
            lines.append("")
        else:
            # Individual failures (when forest is small)
        ...
        # Truncate to max_chars to prevent context explosion
        if len(summary) > max_chars:
            summary = summary[:max_chars - 50] + "\n\n... [truncated — too many experiments]"

        return summary

    def prune_old_entries(self, keep_last_n: int = 200):
        """Archive experiments beyond keep_last_n to prevent unbounded growth."""
        if len(self._entries) <= keep_last_n:
            return 0

        # Sort by timestamp, keep most recent
        sorted_entries = sorted(self._entries, key=lambda x: x.get("timestamp", ""), reverse=True)
        to_keep = sorted_entries[:keep_last_n]
        to_archive = sorted_entries[keep_last_n:]

        # Save archive
        archive_path = self.path.parent / "experiment_forest_archive.json"
        existing_archive = []
        if archive_path.exists():
            import json as _json
            with open(archive_path) as f:
                existing_archive = _json.load(f).get("experiments", [])
        ...
        with open(archive_path, "w") as f:
            ...
            _json.dump({
                ...
                "total_archived": len(existing_archive),
                "experiments": existing_archive,
