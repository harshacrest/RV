...
def _build_inner_prompt(config: MetaConfig) -> str:
    ...
    program_file = SCRIPT_DIR / "program_regime.md"
...
CRITICAL: You must stop after {config.max_inner_experiments} experiments maximum.
...
BEGIN. Run the experiment loop. Stop after {config.max_inner_experiments} experiments.
"""
    return prompt
...
def run_inner_loop(
    ...
    baseline_file = SCRIPT_DIR / "regime_experiment_baseline.py"
