...
- **val_sharpe** (40%): Portfolio Sharpe ratio on held-out validation period (Jul 2025 – Jan 2026)
- **safe_separation** (25%): Average AL% gap between Safe and Exposed states (bigger = better discrimination)
- **rank_stability** (20%): Spearman rank correlation of state ordering between train and validation (higher = more robust)
- **state_coverage** (15%): Penalty if any state has <5 days in validation (ensures usable classification)
